...
export const uploadToImageKitHandler = async (req, res) => {
    try {
        if (!req.file) {
        ...
        const { url, fileId } = await uploadImageToImageKit(req.file.buffer, req.file.originalname, '/hitian-inside-events');
